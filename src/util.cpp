...
bool LogAcceptCategory(const char* category)
{
    if (category != NULL) {
        ...
        if (ptrCategory.get() == NULL) {
            ...
            // "aezora" is a composite category enabling all AEZORA-related debug output
            if (ptrCategory->count(std::string("aezora"))) {
...
{
    boost::filesystem::path pathConfigFile(GetArg("-conf", "aezora.conf"));
...
{
    boost::filesystem::path pathPidFile(GetArg("-pid", "aezorad.pid"));
