...
bool CTxOut::GetKeyIDFromUTXO(CKeyID& keyIDRet) const
{
    ...
    if (whichType == TX_PUBKEYHASH || whichType == TX_COLDSTAKE_LEGACY) {
...
bool CTransaction::CheckColdStakeLegacy(const CScript& script) const
{
...
bool CTransaction::HasP2CSOutputs() const
{
    for(const CTxOut& txout : vout) {
        if (txout.scriptPubKey.IsPayToColdStakingLegacy())
            return true;
    }
    return false;
}
...
CAmount CTransaction::GetValueOut() const
{
    ...
    {
        // AZR: previously MoneyRange() was called here. This has been replaced with negative check and boundary wrap check.
        if (it->nValue < 0)
