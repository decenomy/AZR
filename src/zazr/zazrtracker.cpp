...
CzAZRTracker::~CzAZRTracker()
{
    mapSerialHashes.clear();
    mapPendingSpends.clear();
}

void CzAZRTracker::Init()
{
...
bool CzAZRTracker::Archive(CMintMeta& meta)
{
...
bool CzAZRTracker::UnArchive(const uint256& hashPubcoin, bool isDeterministic)
{
    CWalletDB walletdb(strWalletFile);
    if (isDeterministic) {
        CDeterministicMint dMint;
...
CMintMeta CzAZRTracker::Get(const uint256 &hashSerial)
{
...
CMintMeta CzAZRTracker::GetMetaFromPubcoin(const uint256& hashPubcoin)
{
...
bool CzAZRTracker::GetMetaFromStakeHash(const uint256& hashStake, CMintMeta& meta) const
{
    for (auto& it : mapSerialHashes) {
...
CAmount CzAZRTracker::GetBalance(bool fConfirmedOnly, bool fUnconfirmedOnly) const
{
    CAmount nTotal = 0;
...
CAmount CzAZRTracker::GetUnconfirmedBalance() const
{
    return GetBalance(false, true);
}

std::vector<CMintMeta> CzAZRTracker::GetMints(bool fConfirmedOnly) const
{
...
bool CzAZRTracker::HasMintTx(const uint256& txid)
{
    for (auto it : mapSerialHashes) {
...
bool CzAZRTracker::HasPubcoin(const CBigNum &bnValue) const
{
...
bool CzAZRTracker::HasPubcoinHash(const uint256& hashPubcoin) const
{
    for (auto it : mapSerialHashes) {
        CMintMeta meta = it.second;
        if (meta.hashPubcoin == hashPubcoin)
...
bool CzAZRTracker::HasSerial(const CBigNum& bnSerial) const
{
    uint256 hash = GetSerialHash(bnSerial);
    return HasSerialHash(hash);
}

bool CzAZRTracker::HasSerialHash(const uint256& hashSerial) const
{
    auto it = mapSerialHashes.find(hashSerial);
    return it != mapSerialHashes.end();
}

bool CzAZRTracker::UpdateZerocoinMint(const CZerocoinMint& mint)
{
...
bool CzAZRTracker::UpdateState(const CMintMeta& meta)
{
    CWalletDB walletdb(strWalletFile);
...
void CzAZRTracker::Add(const CDeterministicMint& dMint, bool isNew, bool isArchived, CzAZRWallet* zAZRWallet)
{
    bool iszAZRWalletInitialized = (NULL != zAZRWallet);
    CMintMeta meta;
    ...
    if (! iszAZRWalletInitialized)
        zAZRWallet = new CzAZRWallet(strWalletFile);
    meta.isSeedCorrect = zAZRWallet->CheckSeed(dMint);
    if (! iszAZRWalletInitialized)
        delete zAZRWallet;
    mapSerialHashes[meta.hashSerial] = meta;
...
void CzAZRTracker::Add(const CZerocoinMint& mint, bool isNew, bool isArchived)
{
    CMintMeta meta;
...
void CzAZRTracker::SetPubcoinUsed(const uint256& hashPubcoin, const uint256& txid)
{
...
void CzAZRTracker::SetPubcoinNotUsed(const uint256& hashPubcoin)
{
    if (!HasPubcoinHash(hashPubcoin))
        return;
    CMintMeta meta = GetMetaFromPubcoin(hashPubcoin);
...
void CzAZRTracker::RemovePending(const uint256& txid)
{
    uint256 hashSerial;
...
bool CzAZRTracker::UpdateStatusInternal(const std::set<uint256>& setMempool, CMintMeta& mint)
{
...
std::set<CMintMeta> CzAZRTracker::ListMints(bool fUnusedOnly, bool fMatureOnly, bool fUpdateStatus, bool fWrongSeed, bool fExcludeV1)
{
    CWalletDB walletdb(strWalletFile);
    if (fUpdateStatus) {
        ...
        CzAZRWallet* zAZRWallet = new CzAZRWallet(strWalletFile);
        for (auto& dMint : listDeterministicDB) {
            if (fExcludeV1 && dMint.GetVersion() < 2)
                continue;
            Add(dMint, false, false, zAZRWallet);
        }
        delete zAZRWallet;
        LogPrint("zero", "%s: added %d dzazr from DB\n", __func__, listDeterministicDB.size());
    }
