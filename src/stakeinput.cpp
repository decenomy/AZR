...
CZAzrStake::CZAzrStake(const libzerocoin::CoinSpend& spend)
{
...
int CZAzrStake::GetChecksumHeightFromMint()
{
...
int CZAzrStake::GetChecksumHeightFromSpend()
{
    return GetChecksumHeight(nChecksum, denom);
}

uint32_t CZAzrStake::GetChecksum()
{
    return nChecksum;
}

// The zAZR block index is the first appearance of the accumulator checksum that was used in the spend
// note that this also means when staking that this checksum should be from a block that is beyond 60 minutes old and
// 100 blocks deep.
CBlockIndex* CZAzrStake::GetIndexFrom()
{
    if (pindexFrom)
        return pindexFrom;
...
bool CZAzrStake::GetModifier(uint64_t& nStakeModifier)
{
    ...
    // zAZR staking is disabled long before block v7 (and checkpoint is not included in blocks since v7)
...
CDataStream CZAzrStake::GetUniqueness()
{
    //The unique identifier for a zAZR is a hash of the serial
    CDataStream ss(SER_GETHASH, 0);
    ss << hashSerial;
    return ss;
}

bool CZAzrStake::CreateTxIn(CWallet* pwallet, CTxIn& txIn, uint256 hashTxOut)
{
...
bool CZAzrStake::CreateTxOuts(CWallet* pwallet, std::vector<CTxOut>& vout, CAmount nTotal)
{
    //Create an output returning the zAZR that was staked
    CTxOut outReward;
    libzerocoin::CoinDenomination denomStaked = libzerocoin::AmountToZerocoinDenomination(this->GetValue());
    CDeterministicMint dMint;
    if (!pwallet->CreateZAZROutPut(denomStaked, outReward, dMint))
        return error("%s: failed to create zAZR output", __func__);
    vout.emplace_back(outReward);

    //Add new staked denom to our wallet
    if (!pwallet->DatabaseMint(dMint))
        return error("%s: failed to database the staked zAZR", __func__);

    for (unsigned int i = 0; i < 3; i++) {
        CTxOut out;
        CDeterministicMint dMintReward;
        if (!pwallet->CreateZAZROutPut(libzerocoin::CoinDenomination::ZQ_ONE, out, dMintReward))
            return error("%s: failed to create zAZR output", __func__);
...
bool CZAzrStake::GetTxFrom(CTransaction& tx)
{
    return false;
}

bool CZAzrStake::MarkSpent(CWallet *pwallet, const uint256& txid)
{
    CzAZRTracker* zazrTracker = pwallet->zazrTracker.get();
    CMintMeta meta;
    if (!zazrTracker->GetMetaFromStakeHash(hashSerial, meta))
        return error("%s: tracker does not have serialhash", __func__);

    zazrTracker->SetPubcoinUsed(meta.hashPubcoin, txid);
    return true;
}

//!AZR Stake
bool CAzrStake::SetInput(CTransaction txPrev, unsigned int n)
{
    this->txFrom = txPrev;
    this->nPosition = n;
    return true;
...
bool CAzrStake::GetTxFrom(CTransaction& tx)
{
    tx = txFrom;
    return true;
}
...
bool CAzrStake::GetModifier(uint64_t& nStakeModifier)
{
...
CDataStream CAzrStake::GetUniqueness()
{
