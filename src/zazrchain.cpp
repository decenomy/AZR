...
{
    CDataStream serializedCoinSpend = ZAZRModule::ScriptSigToSerializedSpend(txin.scriptSig);
...
bool UpdateZAZRSupplyConnect(const CBlock& block, CBlockIndex* pindex, bool fJustCheck)
{
    ...
    //Add mints to zAZR supply (mints are forever disabled after last checkpoint)
...
bool UpdateZAZRSupplyDisconnect(const CBlock& block, CBlockIndex* pindex)
{
    ...
    // Re-add spends to zAZR supply
    std::list<libzerocoin::CoinDenomination> listDenomsSpent = ZerocoinSpendListFromBlock(block, true);
    for (const libzerocoin::CoinDenomination& denom : listDenomsSpent) {
        mapZerocoinSupply.at(denom)++;
    }

    // Remove mints from zAZR supply (mints are forever disabled after last checkpoint)
    if (pindex->nHeight < consensus.height_last_ZC_AccumCheckpoint) {
        std::list<CZerocoinMint> listMints;
        std::set<uint256> setAddedToWallet;
        BlockToZerocoinMintList(block, listMints, true);
