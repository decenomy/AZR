...
void PIVXGUI::createTrayIcon(const NetworkStyle* networkStyle)
{
    ...
    QString toolTip = tr("Azzure client") + " " + networkStyle->getTitleAddText();
...
{
    QString strTitle =  tr("Azzure"); // default title
...
bool PIVXGUI::openStandardDialog(QString title, QString body, QString okBtn, QString cancelBtn)
{
    ...
    if (isVisible()) {
        ...
        dialog->setWindowTitle(tr("Azzure"));
        dialog->adjustSize();
        dialog->raise();
        dialog->exec();
    }
