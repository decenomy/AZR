...
#include "qt/aezora/dashboardwidget.h"
#include "qt/aezora/forms/ui_dashboardwidget.h"
#include "qt/aezora/sendconfirmdialog.h"
#include "qt/aezora/txrow.h"
...
DashboardWidget::DashboardWidget(AEZORAGUI* parent) :
    PWidget(parent),
    ui(new Ui::DashboardWidget)
{
    ...
    setCssProperty(ui->labelSquareAzr, "square-chart-azr");
    setCssProperty(ui->labelSquarezAzr, "square-chart-zazr");
    setCssProperty(ui->labelAzr, "text-chart-azr");
    setCssProperty(ui->labelZazr, "text-chart-zazr");
    ui->labelZazr->setVisible(false);
    ui->labelSquarezAzr->setVisible(false);
    ...
    setCssProperty(ui->labelAmountAzr, "text-stake-azr-disable");
    setCssProperty(ui->labelAmountZazr, "text-stake-zazr-disable");
...
bool hasCharts = false;
    ...
    if (window)
        connect(window, &AEZORAGUI::windowResizeEvent, this, &DashboardWidget::windowResizeEvent);
#endif
...
void DashboardWidget::loadWalletModel()
{
    if (walletModel && walletModel->getOptionsModel()) {
        ...
        connect(ui->pushImgEmpty, &QPushButton::clicked, window, &AEZORAGUI::openFAQ);
        connect(ui->btnHowTo, &QPushButton::clicked, window, &AEZORAGUI::openFAQ);
    ...
    // update the display unit, to not use the default ("AZR")
    updateDisplayUnit();
}
...
const QMap<int, std::pair<qint64, qint64>> DashboardWidget::getAmountBy()
{
    ...
    for (int i = 0; i < size; ++i) {
        ...
        bool isAzr = modelIndex.data(TransactionTableModel::TypeRole).toInt() != TransactionRecord::StakeZAZR;
...
bool DashboardWidget::loadChartData(bool withMonthNames)
{
    ...
    chartData->amountsByCache = getAmountBy(); // pair AZR, zAZR
    ...
    for (int j = range.first; j < range.second; j++) {
        ...
        if (chartData->amountsByCache.contains(num)) {
            ...
            azr = (pair.first != 0) ? pair.first / 100000000 : 0;
            zazr = (pair.second != 0) ? pair.second / 100000000 : 0;
            chartData->totalAzr += pair.first;
            chartData->totalZazr += pair.second;
        }

        chartData->xLabels << ((withMonthNames) ? monthsNames[num - 1] : QString::number(num));

        chartData->valuesAzr.append(azr);
        chartData->valueszAzr.append(zazr);

        int max = std::max(azr, zazr);
...
void DashboardWidget::onChartRefreshed()
{
    ...
    set0->append(chartData->valuesAzr);
    set1->append(chartData->valueszAzr);

    // Total
    nDisplayUnit = walletModel->getOptionsModel()->getDisplayUnit();
    if (chartData->totalAzr > 0 || chartData->totalZazr > 0) {
        setCssProperty(ui->labelAmountAzr, "text-stake-azr");
        setCssProperty(ui->labelAmountZazr, "text-stake-zazr");
    } else {
        setCssProperty(ui->labelAmountAzr, "text-stake-azr-disable");
        setCssProperty(ui->labelAmountZazr, "text-stake-zazr-disable");
    }
    forceUpdateStyle({ui->labelAmountAzr, ui->labelAmountZazr});
    ui->labelAmountAzr->setText(GUIUtil::formatBalance(chartData->totalAzr, nDisplayUnit));
    ui->labelAmountZazr->setText(GUIUtil::formatBalance(chartData->totalZazr, nDisplayUnit, true));
