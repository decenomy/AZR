...
#include "qt/aezora/forms/ui_addresseswidget.h"
#include "qt/aezora/addresslabelrow.h"
#include "qt/aezora/addnewaddressdialog.h"
...
AddressesWidget::AddressesWidget(AEZORAGUI* parent) :
    PWidget(parent),
...
void AddressesWidget::onStoreContactClicked(){
    if (walletModel) {
        ...
        CBitcoinAddress azrAdd = CBitcoinAddress(address.toUtf8().constData());
        ...
        if (walletModel->updateAddressBookLabels(azrAdd.Get(), label.toUtf8().constData(),
                azrAdd.IsStakingAddress() ? AddressBook::AddressBookPurpose::COLD_STAKING_SEND : AddressBook::AddressBookPurpose::SEND)
                ) {
