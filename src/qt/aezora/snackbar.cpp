...
#include "qt/aezora/forms/ui_snackbar.h"
#include "qt/aezora/qtutils.h"
#include <QTimer>


SnackBar::SnackBar(AEZORAGUI* _window, QWidget *parent) :
    QDialog(parent),
    ui(new Ui::SnackBar),
    window(_window),
    timeout(MIN_TIMEOUT)
{
    ...
    if (window)
        connect(window, &AEZORAGUI::windowResizeEvent, this, &SnackBar::windowResizeEvent);
    else {
