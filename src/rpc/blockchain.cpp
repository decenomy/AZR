...
UniValue blockToJSON(const CBlock& block, const CBlockIndex* blockindex, bool txDetails = false)
{
    ...
    for (auto denom : libzerocoin::zerocoinDenomList) {
        zazrObj.push_back(Pair(std::to_string(denom), ValueFromAmount(blockindex->mapZerocoinSupply.at(denom) * (denom*COIN))));
    }
    zazrObj.push_back(Pair("total", ValueFromAmount(blockindex->GetZerocoinSupply())));
...
UniValue getrawmempool(const UniValue& params, bool fHelp)
{
    if (fHelp || params.size() > 1)
        throw std::runtime_error(
            ...
            "    \"fee\" : n,              (numeric) transaction fee in aezora\n"
...
UniValue getblock(const UniValue& params, bool fHelp)
{
    if (fHelp || params.size() < 1 || params.size() > 2)
        throw std::runtime_error(
            ...
            "  \"zAZRsupply\" :\n"
            "  {\n"
            "     \"1\" : n,            (numeric) supply of 1 zAZR denomination\n"
            "     \"5\" : n,            (numeric) supply of 5 zAZR denomination\n"
            "     \"10\" : n,           (numeric) supply of 10 zAZR denomination\n"
            "     \"50\" : n,           (numeric) supply of 50 zAZR denomination\n"
            "     \"100\" : n,          (numeric) supply of 100 zAZR denomination\n"
            "     \"500\" : n,          (numeric) supply of 500 zAZR denomination\n"
            "     \"1000\" : n,         (numeric) supply of 1000 zAZR denomination\n"
...
UniValue gettxout(const UniValue& params, bool fHelp)
{
    if (fHelp || params.size() < 2 || params.size() > 3)
        throw std::runtime_error(
            ...
            "     \"addresses\" : [          (array of string) array of aezora addresses\n"
...
UniValue getblockindexstats(const UniValue& params, bool fHelp) {
    if (fHelp || params.size() < 2 || params.size() > 3)
        throw std::runtime_error(
                ...
                "  }\n"
                "  \"txbytes\": xxxxx                (numeric) Sum of the size of all txes (zAZR excluded) over block range\n"
                "  \"ttlfee\": xxxxx                 (numeric) Sum of the fee amount of all txes (zAZR mints excluded) over block range\n"
                "  \"ttlfee_all\": xxxxx             (numeric) Sum of the fee amount of all txes (zAZR mints included) over block range\n"
