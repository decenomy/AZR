...
*Setup instructions for a gitian build of AEZORA Core using a VM or physical system.*

Gitian is the deterministic build process that is used to build the AEZORA
...
to the AEZORA GitHub release page.
...
- [Building AEZORA Core](#building-aezora-core)
...
```bash
gpg --output ${VERSION}-linux/${NAME}/aezora-linux-${VERSION%\.*}-build.assert.sig --detach-sign ${VERSION}-linux/$NAME/aezora-linux-${VERSION%\.*}-build.assert
gpg --output ${VERSION}-osx-unsigned/$NAME/aezora-osx-${VERSION%\.*}-build.assert.sig --detach-sign ${VERSION}-osx-unsigned/$NAME/aezora-osx-${VERSION%\.*}-build.assert
gpg --output ${VERSION}-win-unsigned/$NAME/aezora-win-${VERSION%\.*}-build.assert.sig --detach-sign ${VERSION}-win-unsigned/$NAME/aezora-win-${VERSION%\.*}-build.assert
```

Uploading Signatures
--------------------
Make a Pull Request (both the `.assert` and `.assert.sig` files) to the
[gitian.sigs](https://github.com/Aezora/gitian.sigs/) repository:
...
You can also mail the files to Fuzzbawls (fuzzbawls@aezora.com) and he will commit them.

```bash
gpg --detach-sign ${VERSION}-linux/${NAME}/aezora-linux-*-build.assert
gpg --detach-sign ${VERSION}-win-unsigned/${NAME}/aezora-win-*-build.assert
gpg --detach-sign ${VERSION}-osx-unsigned/${NAME}/aezora-osx-*-build.assert
```
